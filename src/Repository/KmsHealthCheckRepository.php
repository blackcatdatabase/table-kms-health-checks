...
namespace BlackCat\Database\Packages\KmsHealthChecks\Repository;
...
use BlackCat\Database\Packages\KmsHealthChecks\Criteria;
use BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto as Dto;
use BlackCat\Database\Packages\KmsHealthChecks\Mapper\KmsHealthCheckDtoMapper as RowMapper;
...
final class KmsHealthCheckRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KmsHealthChecks\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KmsHealthChecks\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KmsHealthChecks\Mapper\KmsHealthCheckDtoMapper::fromRow($row);
    }
