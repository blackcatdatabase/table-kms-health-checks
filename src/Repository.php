...
namespace BlackCat\Database\Packages\KmsHealthChecks;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KmsHealthChecks\Criteria;
use BlackCat\Database\Packages\KmsHealthChecks\Repository\KmsHealthCheckRepositoryInterface;
use BlackCat\Database\Packages\KmsHealthChecks\Repository\KmsHealthCheckRepository as GeneratedRepository;
...
{
  private KmsHealthCheckRepositoryInterface $repo;
  ...
  public function __construct(private Database $db)
  {
    ...
    public function _setRepositoryForTests(RepoContract $repo): void
    {
        ...
        if (!$repo instanceof KmsHealthCheckRepositoryInterface) {
            // Allow any repo that conforms to the interface even if not generated
            throw new \InvalidArgumentException('Expected KmsHealthCheckRepositoryInterface');
        }
        $this->repo = $repo;
    }
    ...
    public function getByUnique(array $keys, bool $asDto = false): array|\BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto|null { return $this->repo->getByUnique($keys, $asDto); }
    public function getById(int|string|array $id, bool $asDto = false): array|\BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto|null { return $this->repo->getById($id, $asDto); }
