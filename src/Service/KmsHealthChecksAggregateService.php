...
namespace BlackCat\Database\Packages\KmsHealthChecks\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KmsHealthChecks\Dto\KmsHealthCheckDto;
use BlackCat\Database\Packages\KmsHealthChecks\Mapper\KmsHealthCheckDtoMapper;
 ...
 */
final class KmsHealthChecksAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KmsHealthCheckRepository $kmsHealthCheckRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'kms_health_checks' => $this->kmsHealthCheckRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KmsHealthChecks\ModuleException("Record {$idStr} not found for locking.");
            }
