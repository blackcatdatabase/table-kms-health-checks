...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 2 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 2 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\kms-health-checks](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\kms-health-checks)
...
graph LR
  ...
  kms_health_checks["kms_health_checks"]:::center
  kms_health_checks -->|FK| kms_keys["kms_keys"]:::link
  kms_health_checks -->|FK| kms_providers["kms_providers"]:::accent
...
- Outbound (depends on): "kms_keys", "kms_providers"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package kms-health-checks -Apply
